  ...
  <div class="favorite_contents">
    ...
    <div class="ks_result_section">
      <div class="ks_search_form">
        ...
        <button class="btn_search" @click="getProductPrevious(filter, true)">{{ $t('message.search') }}</button>
      </div>
    </div>
...
export default {
  ...
  watch: {
    pageNumber() {
      if (this.currentTab === 2) this.getProductPrevious(this.filter);
    },
  },
  computed: {},
  methods: {
    ...
    getProductPrevious(filter, click = false) {
      ...
      this.$emit('getProductList', { type: 'prev', param, click });
    },
    checkedProduct() {

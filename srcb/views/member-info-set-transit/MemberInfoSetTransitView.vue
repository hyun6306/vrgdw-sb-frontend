      ...
      <div id="MainTabS">
        ...
        <ul class="main-tab-menu">
          <li class="main-tab" v-for="(tab, index) in panes" v-bind:class="{ active: currentTab === index }" :key="index">
            <a v-on:click="(currentTab = index), (activeKey = tab.key)">{{ $t(tab.title) }}</a>
...
import MemberInfoSetTransitRequest from '@wb/views/member-info-set-transit/MemberInfoSetTransitRequest.vue';
// import DevFooter from '@/components/DevFooter.vue';

export default {
  components: { MemberInfoSetTransitRequest, SampleTwo, DashBoard },
  data() {
    const panes = [
      { title: 'label.MemberInfoSetTransitRequest', content: 'MemberInfoSetTransitRequest', key: 'MemberInfoSetTransitRequest' },
    ...
    return {
      ...
      panes,
      newTabIndex: 0,
      selected: 'MemberInfoSetTransitRequest',
      isMainShow: true,
      currentTab: 0,
      main: {
        content: 'DashBoard',

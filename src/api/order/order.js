...
function deleteUserLogitsAddr(param) {
  const { httpUrl, params } = responseUtil.setHttpUrlReplace(urls.ORDER.CUSTOMER.DELETE_USER_LOGITS_BASE_ADDR, param);
  return axios.put(httpUrl, params).then(response => responseUtil.rejectIfNotSuccessful(response));
}
...
function getDeliveryStatusList(param) {
  // 배송현황목록 조회
  const { httpUrl } = responseUtil.setHttpUrlReplace(urls.ORDER.MOBILE_DELIVERY.GET_DELIVERY_STATUS_LIST, param);
  return axios.get(httpUrl).then(resp => responseUtil.rejectIfNotSuccessful(resp));
}

function getDriverDeliveryStatusList(param) {
  // 배송현황목록 조회
  const { httpUrl } = responseUtil.setHttpUrlReplace(urls.ORDER.MOBILE_DELIVERY.GET_DRIVER_DELIVERY_STATUS_LIST, param);
  return axios.get(httpUrl).then(resp => responseUtil.rejectIfNotSuccessful(resp));
}

function saveTakeOver(param) {
  const { httpUrl, params } = responseUtil.setHttpUrlReplace(urls.ORDER.MOBILE_DELIVERY.PUT_SAVE_TAKE_OVER, param);
  return axios.put(httpUrl, params).then(resp => responseUtil.rejectIfNotSuccessful(resp));
}

function getDriverCount(param) {
  const { httpUrl, params } = responseUtil.setHttpUrlReplace(urls.ORDER.MOBILE_DELIVERY.GET_DRIVER_COUNT, param);
  return axios.get(httpUrl, params).then(resp => responseUtil.rejectIfNotSuccessful(resp));
}

export default {
  deleteUserLogitsAddr,
  getUserOfferInfo,

<template>
  <div :class="`checkbox_multiselect ${rand}`">
...
export default {
  props: ['options', 'all', 'default', 'dataKey', 'page', 'disabled'],
  data() {
    return {
      statCd: [],
      cdOptions: [],
  ...
  methods: {
    ...
    isOpen() {
      ...
      window.addEventListener('click', this.checkParent);
    },
    checkParent(event) {
      ...
      if (!existCheckboxMultiSelect) {
        this.isClose(this);
        window.removeEventListener('click', this.checkParent);
      }
    },
    isClose: _.debounce(vm => {
      if (vm.checkboxMultiselect) {
        vm.checkboxMultiselect = false;
        window.removeEventListener('click', vm.checkParent);
      } else {
        vm.isOpen();
      }
    }, 30),

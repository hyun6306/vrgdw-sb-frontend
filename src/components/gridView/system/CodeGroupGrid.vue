    ...
    initGrid() {
      ...
      this.realGrid.gridView.onCellClicked = (grid, clickData) => {
        if (clickData.cellType === 'data') {
          let codeGroup = grid.getValues(clickData.itemIndex);
          if (codeGroup.groupCd !== undefined && codeGroup.groupNm !== undefined) {
            this.disptColor(clickData.dataRow);
            this.$emit('codeGroup', codeGroup);
          }
        }
      };
    },

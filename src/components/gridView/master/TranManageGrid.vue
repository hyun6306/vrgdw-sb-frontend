...
import { fields, columns, layout } from '@/consts/gridData/master/tranManage';
...
export default {
  ...
  methods: {
    async getList(params) {
      try {
        let param = {};
        param = params == null ? { trncusId: '', operClsCd: '', filter: '' } : params;
      ...
      if (this.realGrid) {
        ...
        this.realGrid.gridView.onCellClicked = function (grid, clickData) {
          if (clickData.cellType == 'data') {
            if (clickData.fieldName == 'trncusCd' && vm.authButton.savAuth == 'Y') {
    ...
    async layoutSetting() {
      if (this.savePage.length > 0 && this.savePage !== null) {
        ...
        if (this.savePage[key].storeCls == 'tranMng') {

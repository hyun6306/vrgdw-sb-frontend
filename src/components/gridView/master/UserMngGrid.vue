...
import { fields, columns, layout } from '@/consts/gridData/master/userMng';
...
export default {
  ...
  watch: {
    listData() {
      ...
      //   if (list.language == 'ko') {
      //     list.language = '한국어';
      //   } else if (list.language == 'en') {
      //     list.language = '영어';
      //   } else if (list.language == 'vn') {
      //     list.language = '베트남어';
      //   }
      // });
      this.listData.forEach(list => {
        list.loginBtn = this.$t('message.changeLogin');
      });
  ...
  methods: {
    async initGrid() {
      ...
      if (this.realGrid) {
        ...
        this.realGrid.gridView.setDisplayOptions({
          ...
          emptyMessage: this.$t('message.gridUserListNull'),
        });
        ...
        this.realGrid.gridView.onCellClicked = function (grid, clickData) {
          if (clickData.cellType == 'data') {
            if (clickData.fieldName == 'userNm' && vm.authButton.savAuth == 'Y') {
              ...
              vm.$parent.showInfoModal(clickedRow);
            }
          }
        };
      }
      await this.layoutSetting();
      this.realGrid.gridView.onCellItemClicked = (grid, index, clickData) => {
        if (clickData.fieldName === 'loginBtn') {
          const userInfo = this.realGrid.dataProvider.getJsonRow(index.dataRow);
          this.$emit('getOtherLogin', userInfo);
        }
        return true;
      };
    },
    async layoutSetting() {
      if (this.savePage.length > 0 && this.savePage !== null) {
        ...
        if (this.savePage[key].storeCls == 'userMng') {
    ...
    async getList(params) {
      try {
        let param = {};
        param = params !== null ? params : { filter: '', menuGrpCd: '', roles: '', degreeCd: '' };
        const response = await masterApi.getUserList(param);

<template>
  <div :id="gridName" ref="GeneralSales" style="width: 100%"></div>
</template>

<script>
import RealGridCreate from '@/mixins/realGridCreate';
import { fields, columns, layout, baseColumn, ordColumn } from '@/consts/gridData/order/generalSalesList';
  ...
  methods: {
    ...
    getCheckedRowsData() {
      ...
      selectProductIndexs.forEach(data => {
        let row = this.realGrid.dataProvider.getJsonRow(data);
        this.setSelectProductRows.push({
          custId: row.custId,
          id: row.id,
          offrStatCd: row.offrStatCd,
          apprCfmYn: row.apprCfmYn,
          offrNum: row.offrNum,
        });
      });

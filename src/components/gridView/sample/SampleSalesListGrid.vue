...
import { fields, columns, layout, baseColumn, ordColumn } from '@/consts/gridData/sample/sampleSalesList';
...
export default {
  ...
  watch: {
    refreshGrid() {
      ...
      if (this.cancelId && this.userMenuGrpCd !== 'CUSTOMER') {
        this.setCancelRow();
      }
    },
    gridLang() {
    ...
    async layoutSetting() {
      if (this.savePage.length > 0 && this.savePage !== null) {
        ...
        if (this.savePage[key].storeCls == 'sampleList') {
          let loadLayout = this.savePage[key].uiJson;

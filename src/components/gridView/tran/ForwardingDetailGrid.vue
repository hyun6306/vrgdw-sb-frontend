...
import { fields, columns, layout } from '@/consts/gridData/tran/forwardingDetail';
...
export default {
  props: ['listRows', 'gridLang', 'savePage', 'page', 'gridName'],
  data() {
    return {
      // gridName: 'forwardingDetail',
      realGrid: null,
    };
  },
  computed: {},
  ...
  methods: {
    calculator(rows) {
      const qtyInfo = {
        length: rows.length,
        checkedQty: roundToTwo(rows.reduce((acc, curr) => acc + Number(curr.cfmOrdQty ?? 0), 0)),

      ...
      <div class="search_box_filter">
        <div class="filter_list">
          ...
          <label for="" class="filter_type">
            ...
            <!-- 데이터피커 -->
            <input type="text" style="" id="custoer_name" :placeholder="$t('message.msgPlaceApiLog')" v-model="filter" ref="filter" @keyup.enter="[resetPageNumber(), getApiLog()]" />
          </label>
          <div style="display: flex; margin-left: auto">
            <button class="btn_reset" style="margin-right: 10px" @click="reset">{{ $t('message.btnInit') }}</button>
            <button class="btn_search_text" @click="[resetPageNumber(), getApiLog()]">{{ $t('message.btnSearch') }}</button>
          </div>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    async getApiLog() {
      ...
      try {
        ...
        const resp = await systemApi.getApiLog(param);

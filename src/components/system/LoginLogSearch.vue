      ...
      <div class="search_box_filter">
        <div class="filter_list">
          ...
          <label for="" class="filter_type">
            ...
            <input type="text" style="" id="custoer_name" :placeholder="$t('message.msgPlaceLoginLog')" v-model="filter" ref="filter" @keyup.enter="[resetPageNumber(), getLoginLog()]" />
          </label>
          <div style="display: flex; margin-left: auto">
            <button class="btn_reset" style="margin-right: 10px" @click="reset">{{ $t('message.btnInit') }}</button>
            <button class="btn_search_text" @click="[resetPageNumber(), getLoginLog()]">{{ $t('message.btnSearch') }}</button>
          </div>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    async getLoginLog() {
      ...
      try {
        ...
        const resp = await systemApi.getLoginLog(param);

  ...
  <div>
      ...
      <div class="search_box">
        <div class="search_box_filter">
          <div class="filter_list">
            ...
            <button class="btn_search_text ml-0" @click="monthSearch('search')">{{ $t('message.search') }}</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="real_footer">
      ...
      <div class="footer_right">
        <button class="btn_default btn_critical_process" @click="chgBlock">{{ $t('message.chgBlock') }}</button>
      </div>
    </div>
    <CalendarOutModal ref="CalendarOutModal" />
...
import CalendarOutModal from '@/modal/master/CalendarOutModal';
  ...
  methods: {
    ...
    monthSearch(type) {
      if (type == 'reset') {
        ...
        //2. 이동한 달의 제한일 목록 api불러오기
        this.trnBlockList();
      }
    },
    // 제한일 추가
    chgBlock() {
      this.$modal.show('CalendarOutModal', { data: '' });
    },
    //모달
    async showModal(data) {
      if (data.type == 'close') {
        await this.$refs.CalendarOutModal.modalClose();
      }
      this.ment = data.ment;
    ...
    handleEventClick(clickInfo) {
      ...
      this.blockdays.forEach(data => {
        if (data.id == id) {
          this.clickedEvent = data;
          this.$modal.show('CalendarOutModal', { data: data });
        }
      });
    },
    handleEvents(events) {
    ...
    addevent(data) {
      let title = data.trnPsbYn == 'Y' ? 'message.trnPsbY' : 'message.trnPsbN';
      let classType = data.trnPsbYn == 'Y' ? 'fc-event-able' : 'fc-event-unable';
    ...
    async trnBlockList() {
      ...
      try {
        ...
        this.blockdays = response.data.ds;
        this.blockdays.forEach(data => {
          data;
          this.addevent(data);
        });

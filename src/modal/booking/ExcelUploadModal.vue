<template>
  <modal name="BookingExcelUploadModal" width="840" height="auto" @before-close="close()" :clickToClose="false">
    <div class="modal" id="ShippingAddress">
      <div slot="top-right" class="btn_modal_close_t02">
        <button @click="$modal.hide('BookingExcelUploadModal')"></button>
      </div>
      ...
      <div class="modal_content excel_content">
        ...
        <div class="modal_btn_contents_t05">
          <button class="btn_default" @click="$modal.hide('BookingExcelUploadModal')">{{ $t('message.btnClose') }}</button>
        </div>
      </div>
    </div>
  </modal>
  ...
  methods: {
    ...
    async upload() {
      ...
      if (!this.file) {
        ...
        try {
          const resp = await bookingApi.saveBookingExcel(formData);

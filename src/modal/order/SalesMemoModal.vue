<template>
  <modal name="SalesMemoModal" width="540" height="auto" @opened="getMemo" :clickToClose="false">
    <div class="modal" id="ShippingSpecifics">
      <div slot="top-right" class="btn_modal_close_t02">
        <button @click="$modal.hide('SalesMemoModal')"></button>
      </div>
      <!-- <SalesMemoModal /> -->
      ...
      <div class="modal_content a-c">
        ...
        <div class="modal_btn_contents_t05">
          <button class="btn_default" v-if="this.userMenuGrpCd !== 'CUSTOMER'" @click="$modal.hide('SalesMemoModal')">{{ $t('message.btnCancel') }}</button>
    ...
    async putSalesMemo(changeRegistYn) {
      try {
        ...
        if (this.registYn == true) {
          ...
          await productApi.putSalesMemo(param);
          changeRegistYn = 'Y';
          this.$modal.hide('SalesMemoModal');

  ...
  <modal name="ExcelUploadModal" width="840" height="auto" @before-close="close()" :clickToClose="false">
    <div class="modal" id="ShippingAddress">
      ...
      <div class="modal_content excel_content">
        ...
        <span class="sub_txt">{{ message }}</span>
        <span class="sub_txt">{{ $t(ment) }}</span>
...
export default {
  ...
  props: ['excelUploadAfter', 'userCustId'],
  data() {
    return {
      fileNm: [],
      errorList: [],
  ...
  methods: {
    ...
    async upload() {
      if (!this.file) {
        ...
        formData.append('custId', this.userCustId);
        try {
          const resp = await orderApi.saveUserOfferExcel(formData);

          this.errorList = resp.data.ds;
          this.message = resp.data.message;
          if (this.errorList.length === 0) {
            this.ment = 'message.msgCheckOrderIst';
          }
        } catch (e) {
          this.errorList = [];

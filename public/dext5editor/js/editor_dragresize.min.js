...
DextDragResize.prototype.ifameOverDivClick=function(b){with(DEXTTOP.G_CURREDITOR.dext_dragresize)try{var a=G_mouseOveriframeObj;resizeHandleClear();G_dextResize_OverElem=null;G_dextResize_elem=a;clearInterval(dextResizeIconInterval);setResizeHandle(b,!0);DEXTTOP.DEXT5.browser.ie||setTimeout(function(){restoreSelection(saveSelection(DEXTTOP.G_CURREDITOR.ID,!0),!0)},0)}catch(c){}};
...
k.body.appendChild(d);G_dextResizeDiv=d;d="dextResize";var n=G_dextResize_elem.tagName.toLowerCase();if("img"==n){var r,p;if(-1<G_dextResize_elem.style.width.indexOf("%")||-1<G_dextResize_elem.style.height.indexOf("%")){var h=getClientRect(G_dextResize_elem);r=h.right-h.left;p=h.bottom-h.top}else""!=G_dextResize_elem.style.width?r=DEXTTOP.DEXT5.util.parseIntOr0(G_dextResize_elem.style.width):(h=getClientRect(G_dextResize_elem),r=h.right-h.left),""!=G_dextResize_elem.style.height?p=DEXTTOP.DEXT5.util.parseIntOr0(G_dextResize_elem.style.height):
(h=getClientRect(G_dextResize_elem),p=h.bottom-h.top);G_ImageR=1*r/p;DEXTTOP.G_SELECTED_IMAGE_ELEMENT=G_dextResize_elem;d=GetParentbyTagName(G_dextResize_elem,"figure")?"focusFigure":"focusImage"}else"table"==n&&(d="focusTable");DEXTTOP.G_CURREDITOR._iconEnable(d);DEXTTOP.DEXT5.util.removeEvent(G_dextResizeDiv,"keydown",keyDown);DEXTTOP.DEXT5.util.addEvent(G_dextResizeDiv,"mousedown",startDragElem);DEXTTOP.DEXT5.util.addEvent(G_dextResizeHandleHolder,"mousedown",startDragElem)}a&&startDragElem(b,
c)}catch(q){}};
...
getClientRect(_iframeWin.frameElement),l=getWindowScrollPos(_iframeWin);0==G_UseMouseTableInoutdent&&(isNaN(parseInt(G_dextResize_elem.style.left,10))&&(G_dextResize_elem.style.left=d-k.left+l[0]-getClientRect(G_dextResize_elem).left+"px"),isNaN(parseInt(G_dextResize_elem.style.top,10))&&(G_dextResize_elem.style.top=e-k.top+l[1]-getClientRect(G_dextResize_elem).top+"px"));var m=getParentPosition();if(G_UseMouseTableInoutdent){var n=d-k.left-m.left;0>n&&(n=-1);if("1"==DEXTTOP.G_CURREDITOR._config.limitTableInoutdent){var c=
0,r=getClientRect(G_dextResize_elem);if(""!=_iframeDoc.body.style.width&&-1<_iframeDoc.body.style.width.indexOf("px"))c=DEXTTOP.DEXT5.util.parseIntOr0(_iframeDoc.body.style.width);else{c=_iframeDoc.documentElement.clientWidth;if(0==c||null==c||void 0==c)c=_iframeDoc.body.clientWidth;c=Math.floor(c);c-=20}if(0<c){var p=c-(r.right-r.left);0>p&&(p=0);G_dextResize_elem.style.marginLeft=n<=p?n+1+"px":p+"px"}else G_dextResize_elem.style.marginLeft=n+1+"px"}else G_dextResize_elem.style.marginLeft=n+1+"px"}else G_dextResize_elem.style.left=
...
DEXTTOP.DEXT5.util.parseIntOr0(l.borderBottomWidth),n=DEXTTOP.DEXT5.util.parseIntOr0(G_dextResize_elem.style.paddingTop)+DEXTTOP.DEXT5.util.parseIntOr0(G_dextResize_elem.style.paddingBottom),r=G_dextResize_elem.style.width,p=G_dextResize_elem.style.height,h,f=0;if(-1<r.indexOf("%")||-1<p.indexOf("%")){var q=getClientRect(G_dextResize_elem);h=DEXTTOP.DEXT5.util.parseIntOr0(q.right-q.left);f=DEXTTOP.DEXT5.util.parseIntOr0(q.bottom-q.top)}else if(h=DEXTTOP.DEXT5.util.parseIntOr0(r,10),f=DEXTTOP.DEXT5.util.parseIntOr0(p,
10),0==h||0==f)q=getClientRect(G_dextResize_elem),0==h&&(h=DEXTTOP.DEXT5.util.parseIntOr0(q.right-q.left)),0==f&&(f=DEXTTOP.DEXT5.util.parseIntOr0(q.bottom-q.top));var t=handle&&handle.className&&handle.className.match(new RegExp(resizeName+"-([tmblr]{2})"))?RegExp.$1:"";if("IMG"==G_dextResize_elem.tagName&&(-1<t.indexOf("br")||-1<t.indexOf("bl")||-1<t.indexOf("tl")||-1<t.indexOf("tr")))h=parseInt(elmW,10)-(m+n),setNodeWidthHeight(G_dextResize_elem,{width:h+"px",height:DEXTTOP.DEXT5.util.parseIntOr0(1*
h/G_ImageR)+"px"});else{if(0<=t.indexOf("l")||0<=t.indexOf("r"))h=parseInt(elmW,10)-(m+n),setNodeWidthHeight(G_dextResize_elem,{width:h+"px"}),""!=G_dextResize_elem.style.height||!G_dextResize_elem.getAttribute("height")&&""!=G_dextResize_elem.getAttribute("height")||setNodeWidthHeight(G_dextResize_elem,{height:f+"px"});if(0<=t.indexOf("t")||0<=t.indexOf("b"))f=parseInt(elmH,10)-(m+n),setNodeWidthHeight(G_dextResize_elem,{height:f+"px"}),""!=G_dextResize_elem.style.width||!G_dextResize_elem.getAttribute("width")&&
""!=G_dextResize_elem.getAttribute("width")||setNodeWidthHeight(G_dextResize_elem,{width:h+"px"})}"1"==DEXTTOP.G_CURREDITOR._config.dragResizeMovable&&(G_dextResize_elem.style.left=parseInt(elmX,10)+"px",G_dextResize_elem.style.top=parseInt(elmY,10)+"px");m=!1;if("table"==G_dextResize_elem.tagName.toLowerCase()){for(var m=!0,n=G_dextResize_elem,C=DEXTTOP.DEXT5.util.parseIntOr0(G_dextResize_elem.style.width,10),D=DEXTTOP.DEXT5.util.parseIntOr0(G_dextResize_elem.style.height,10),r=0,A=n.rows.length,
p=0;p<A;p++)var v=n.rows[p],r=r+parseFloat(v.cells[0].style.height.replace("px",""));for(p=0;p<A;p++){var v=n.rows[p],x=v.cells.length;for(h=q=0;h<x;h++)var u=v.cells[h],q=q+parseFloat(u.style.width.replace("px",""));if((0<=t.indexOf("l")||0<=t.indexOf("r"))&&C!=q)for(q=(q-C)/x,h=0;h<x;h++){var u=v.cells[h],G=Math.floor(100*parseFloat(parseFloat(u.style.width.replace("px",""))-q+parseFloat(d/x)))/100;u.style.width=G+"px"}if((0<=t.indexOf("t")||0<=t.indexOf("b"))&&D!=r)for(q=(r-D)/A,h=0;h<x;h++){var u=
v.cells[h],H=Math.floor(100*parseFloat(parseFloat(u.style.height.replace("px",""))-q+parseFloat(e/A)))/100;u.style.height=H+"px"}}}setHandlePosition();G_dextResizeHandleHolder.dext_resizeinfo.style.display="";G_dextResizeHandleHolder.dext_resizeinfo.style.zIndex=1E6;G_dextResizeHandleHolder.dext_resizeinfo.style.top=handle.offsetTop+"px";G_dextResizeHandleHolder.dext_resizeinfo.style.left=handle.offsetLeft+"px";G_dextResizeHandleHolder.dext_resizeinfo.firstChild&&G_dextResizeHandleHolder.dext_resizeinfo.removeChild(G_dextResizeHandleHolder.dext_resizeinfo.firstChild);
var E=document.createElement("span"),B=DEXTTOP.DEXT5.util.parseIntOr0(getNodeWidthHeight(G_dextResize_elem,"width"),10),w=DEXTTOP.DEXT5.util.parseIntOr0(getNodeWidthHeight(G_dextResize_elem,"height"),10);if(7!=DEXTTOP.DEXT5.browser.ieVersion){var y=getNodeWidthHeight(G_dextResize_elem,"maxWidth");""!=y&&(y=DEXTTOP.DEXT5.util.parseIntOr0(y,10),B=B>y?y:B);var z=getNodeWidthHeight(G_dextResize_elem,"maxHeight");""==z||m||(z=DEXTTOP.DEXT5.util.parseIntOr0(z,10),w=w>z?z:w)}if(0==w)var F=getClientRect(G_dextResize_elem),
w=DEXTTOP.DEXT5.util.parseIntOr0(F.bottom-F.top);E.innerHTML=B+" X "+w;G_dextResizeHandleHolder.dext_resizeinfo.appendChild(E)}G_dextResizeDiv&&(this.setDragResizeDivPosition(G_dextResizeDiv),this.setDragResizeDivSize(G_dextResizeDiv));DEXTTOP.DEXT5.util.cancelEvent(b)}catch(I){}};
...
k=l.bottom-l.top);G_ImageR=1*g/k;var m=G_dextResize_elem.parentNode;if(!DEXTTOP.DEXT5.browser.chrome&&!DEXTTOP.DEXT5.browser.opera){if(G_dextResize_elem.parentNode.tagName&&"figure"==G_dextResize_elem.parentNode.tagName.toLowerCase()){var n=G_dextResize_elem.parentNode.getElementsByTagName("figcaption");n&&n[0]&&(m=getMyFirstOrLastChildElementNode(n[0]),rangy.getSelection(_iframeWin).removeAllRanges(),doSetCaretPosition(m,0))}DEXTTOP.DEXT5.browser.ie&&9>DEXTTOP.DEXT5.browser.ieVersion&&(DEXTTOP.G_CURREDITOR.dext_dragresize.preventScrollMove(),
...
DEXTTOP.G_CURREDITOR._config.event.mouse("mouseup",b,G_dextResize_elem):DEXTTOP.DEXTWIN.dext_mouse_event("mouseup",b,G_dextResize_elem)}catch(r){}if(1==DEXTTOP.DEXT5.config.MouseEventCancel){DEXTTOP.DEXT5.util.cancelEvent(b);return}}docMouseY=docMouseX=topMouseY=topMouseX=null}catch(p){}};
